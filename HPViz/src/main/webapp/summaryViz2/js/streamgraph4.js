...
var xAxisText = ["-15 to -10","-10 to -5","-5 to 0","0 to 5","5 to 10","10 to 15", "All"];
	...
	d3.json("data/summary.json", function(error, dataAll) {
		if(error){
			...
			for(var i = 2; i < 7; i++){
				...
				initThemeRiverAxis2(window['dataCombinedArea'+i]);
				...
				window['svgTR'+i].selectAll("path").attr("transform", function(d){ return "translate("+ ((maxAll/2)-(window['maxYAfterStacked'+i]/2)) + "," + 0 + ")"; });
			...
			/*var newDataPosArea6 = area6Data.filter(isPositive),
				newDataNetArea6 = area6Data.filter(isNeutral),
				newDataNegArea6 = area6Data.filter(isNegative);
			
			dataCombinedArea6 = [{type:eventType[0], dataArr: newDataNegArea6, color:trColor[0]},
							{type:eventType[1], dataArr: newDataNetArea6, color:trColor[1]},
							{type:eventType[2], dataArr: newDataPosArea6, color:trColor[2]}];
			
			stackData6 = stack(dataCombinedArea6);*/
			
			initThemeRiverAxis3(dataCombinedArea6);
			...
			maxYAfterStacked7 = getMaxY2(stackData6);
			svgTR7.selectAll("path").attr("transform", function(d){ return "translate("+ ((maxAll/2)-(maxYAfterStacked7/2)) + "," + 0 + ")"; });
...
function drawThemeRiverChart(){
	...
	maxAll = getMaxY(stackData1);
	svgTR.selectAll("path").attr("transform", function(d){ return "translate("+ ((maxAll/2)-(maxYAfterStacked/2)) + "," + 0 + ")"; });
