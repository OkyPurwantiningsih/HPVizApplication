...
function DrawGraph(){
	console.log("the name of the patient:"+patientName);
	...
	//d3.json("data/summary.json", function(error, dataAll) {
		if(error){
			...
			for(var i = 2; i < 6; i++){
				...
				window['svgTR'+i].selectAll("path").attr("transform", function(d){ return "translate("+ ((maxX/2)-(window['maxYAfterStacked'+i]/2)) + "," + 0 + ")"; });
			...
			svgTR6.selectAll("path").attr("transform", function(d){ return "translate("+ ((maxX/2)-(maxYAfterStacked6/2)) + "," + 0 + ")"; });
...
function drawChartArea1(){
	...
	svgTR.selectAll("path").attr("transform", function(d){ return "translate("+ ((maxX/2)-(maxYAfterStacked/2)) + "," + 0 + ")"; });
