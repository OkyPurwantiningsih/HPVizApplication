...
function DrawGraph(){
	...
	//d3.json("data/file1AllInfo.json", function(error, dataAll) {
		if(error){
			console.log(error);
...
function generateDataCombinedDef(){
	...
	if((newDataNegGrouped.length == 0) && (newDataNetGrouped.length == 0) && (newDataPosGrouped.length == 0)){
		//console.log("Only Pos");
		newDtCombined = [{type:eventType[0], dataArr: emptyArr, color:trColor[0]},
						{type:eventType[1], dataArr: emptyArr, color:trColor[1]},
						{type:eventType[2], dataArr: emptyArr, color:trColor[2]}];
	}
